<template>
  <div class="p-2 h-full flex flex-col">
    ...
    <FormModal 
      :show="showModal" 
      :title="editingType ? 'Edit Connection Type' : 'Add New Connection Type'" 
      :loading="saving"
      @close="closeModal"
      @submit="saveType"
      maxWidth="md"
    >
      <div class="space-y-4">
        ...
        <div>
            ...
            <input v-model="form.name" type="text" class="w-full text-sm text-gray-700 border border-gray-200 rounded px-3 py-2 leading-tight focus:outline-none focus:border-[#00bcd4] transition-colors" placeholder="e.g. PPPoE">
...
const fetchTypes = async (page = 1) => {
  loading.value = true;
  try {
    const response = await axios.get('/api/config/connection-types', {
...
const saveType = async () => {
  ...
  try {
    if (editingType.value) {
      await axios.put(`/api/config/connection-types/${editingType.value.id}`, form.value);
      toast.fire({ icon: 'success', title: 'Connection Type updated successfully' });
    } else {
      await axios.post('/api/config/connection-types', form.value);
      toast.fire({ icon: 'success', title: 'Connection Type created successfully' });
    }
    closeModal();
    ...
    if (error.response && error.response.status === 422) {
      ...
      console.error('Error saving connection type:', error);
       toast.fire({ icon: 'error', title: 'Failed to save connection type' });
    }
  } finally {
    saving.value = false;
  }
...
const deleteType = async (item) => {
    const result = await confirmAlert('Are you sure?', 'You verify deleting this connection type!');
    if (result.isConfirmed) {
        try {
            await axios.delete(`/api/config/connection-types/${item.id}`);
            toast.fire({ icon: 'success', title: 'Connection type deleted successfully' });
            fetchTypes(pagination.value.current_page);
        } catch (error) {
            toast.fire({ icon: 'error', title: 'Failed to delete connection type' });
            console.error(error);
        }
    }
};
